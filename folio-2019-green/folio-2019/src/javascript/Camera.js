...
export default class Camera
{
    ...
    setAngle()
    {
        ...
        this.angle.items = {
            ...
            projectsClose: new THREE.Vector3(0.05, - 0.95, 0.42)
        }
    ...
    setInstance()
    {
        ...
        this.time.on('tick', () =>
        {
            ...
            {
                const activeTarget = this.focus.active ? this.focus.target : this.target

                this.targetEased.x += (activeTarget.x - this.targetEased.x) * this.easing
                this.targetEased.y += (activeTarget.y - this.targetEased.y) * this.easing
                this.targetEased.z += (activeTarget.z - this.targetEased.z) * this.easing
    ...
    setZoom()
    {
        ...
        this.zoom.defaultTargetValue = this.zoom.targetValue
    ...
    setFocus(_options)
    {
        ...
        this.focus.target = new THREE.Vector3(_options.target.x, _options.target.y, _options.target.z || 0)
        this.pan.reset()

        if(_options.angle)
        {
            this.angle.set(_options.angle)
        }

        if(typeof _options.zoom === 'number')
        {
            this.zoom.targetValue = _options.zoom
        }
    }
    ...
    clearFocus()
    {
        ...
        this.angle.set('projects')
        this.zoom.targetValue = this.zoom.defaultTargetValue
        this.pan.reset()
    }
}
